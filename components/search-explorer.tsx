...
export function SearchExplorer({ events, collections, guides }: { events: HobbyEvent[]; collections: Collection[]; guides: Guide[] }) {
  ...
  return (
    <div className="search-explorer">
      ...
      <div className="search-explorer__field"><MagnifyingGlass size={25} /><input id="site-search" className="search-input" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Try event, binder, or card care" autoFocus /></div>
